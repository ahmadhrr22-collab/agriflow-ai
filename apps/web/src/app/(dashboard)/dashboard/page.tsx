...
import { useRegions }            from '@/hooks/use-prices';
import { useDashboardStore }     from '@/store/dashboard.store';

function DashboardInit() {
  const { data: commodities } = useCommodities();
  const { data: regions }     = useRegions();
  const { initializeIfEmpty } = useDashboardStore();

  useEffect(() => {
    if (commodities?.length > 0 && regions?.length > 0) {
      // Cari Jakarta Pusat, fallback ke region pertama
      const jakartaPusat = regions.find((r: any) => r.name === 'Jakarta Pusat');
      const cabaiMerah   = commodities.find((c: any) => c.name === 'cabai-merah');
      initializeIfEmpty(
        cabaiMerah?.id || commodities[0].id,
        jakartaPusat?.id || regions[0].id,
      );
    }
  }, [commodities, regions, initializeIfEmpty]);
